...
import {WelcomeProxyImpl} from './beacon_welcome_proxy.js';
...
export class WelcomeAppElement extends PolymerElement {
  static get is() {
    return 'welcome-app';
  }
  ...
  constructor() {
    ...
    // There are no subpages in Welcome. Also remove the query param here
  ...
  override connectedCallback() {
    ...
    this.eventTracker_.add(window, 'message', (event: MessageEvent) => {
      ...
      if (event.data == 'done') {
        console.log('navigating to new tab');
   ...
   * @param url The Welcome URL to use in the iframe.
   */
