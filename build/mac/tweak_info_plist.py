...
def _OverrideVersionKey(plist, beacon_version):
    ...
    version_values = beacon_version.split('.')
...
def Main():
    ...
    parser.add_argument('--beacon_channel', dest='beacon_channel', action='store',
        default=None, help='Channel (beta, dev, nightly)')
    parser.add_argument('--beacon_product_dir_name', dest='beacon_product_dir_name',
        action='store', default=None,
        help='Product directory name')
    parser.add_argument('--beacon_version', dest='beacon_version', action='store',
        default=None, help='beacon version string')
    ...
    if args.skip_signing and args.beacon_channel != "":
        plist['KSChannelID'] = args.beacon_channel
    elif 'KSChannelID' in plist:
        # 'KSChannelID' is set at _modify_plists() of modification.py only
        # during signing
        del plist['KSChannelID']

    plist['CrProductDirName'] = args.beacon_product_dir_name

    _OverrideVersionKey(plist, args.beacon_version)
